...
public class DeleteInstanceMVCActionCommandTest {
	...
	@Test
	public void testProcessActionEnqueuesTheDelete() throws Exception {
		JSONObject jsonObject = _getResponseJSONObject(
			_processAction(TestPropsValues.getUser()));

		Assert.assertEquals(
			"The virtual instance " + _company.getWebId() +
				" is being deleted. You will be notified when it is done.",
		...
		Assert.assertNull(
			_companyLocalService.fetchCompany(_company.getCompanyId()));

		_company = null;
	}

	@Test
	public void testProcessActionWhenDeleteIsAlreadyRunning() throws Exception {
		...
		Assert.assertEquals(
			"This virtual instance is already being deleted.",
	...
	public void testProcessActionWhenUserIsNotOmniadmin() throws Exception {
		...
		JSONObject jsonObject = _getResponseJSONObject(_processAction(_user));
	...
	private com.liferay.portal.background.task.model.BackgroundTask
		...
		backgroundTask.setName(
			"DeleteVirtualInstance#" + _company.getCompanyId());
	...
	private BackgroundTask _fetchBackgroundTask() {
		for (BackgroundTask backgroundTask :
			...
			if (name.endsWith("#" + _company.getCompanyId())) {
	...
	private MockLiferayPortletActionResponse _processAction(User user)
		throws Exception {
		...
		mockLiferayPortletActionRequest.setParameter(
			"companyId", String.valueOf(_company.getCompanyId()));
	...
	@Inject(filter = "mvc.command.name=/portal_instances/delete_instance")
