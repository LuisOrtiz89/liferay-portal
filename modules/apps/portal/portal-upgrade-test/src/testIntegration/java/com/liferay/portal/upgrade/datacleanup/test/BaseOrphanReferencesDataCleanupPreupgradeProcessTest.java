...
public class BaseOrphanReferencesDataCleanupPreupgradeProcessTest {
	...
	public void testUpgrade() throws Exception {
		...
		try (Connection connection = DataAccess.getConnection();
			...
			UpgradeProcess upgradeProcess =
				new BaseOrphanReferencesDataCleanupPreupgradeProcess("companyId", "Company");
			...
			Assert.assertEquals(
				_getExpectedMessage(companyId1, 2, dbInspector),
				logEntry.getMessage());

			logEntry = logEntries.get(1);

			Assert.assertEquals(
				_getExpectedMessage(companyId2, 1, dbInspector),
				logEntry.getMessage());
		}
	}
	...
	private String _getExpectedMessage(
		...
		return StringBundler.concat(
			...
			dbInspector.normalizeName("Portlet"),
